...
const Search = () => {
  ...
  return (
    <div className="w-full bg-purple-300 h-full p-2 md:rounded overflow-y-auto">
      <div className="flex gap-2 mb-2">
        <button
          className={`border-2 rounded-3xl px-2 py-1 transition-all duration-150 ${
            show === "posts"
              ? "text-black bg-purple-300 shadow-lg"
          ...
          <div className="w-full grid grid-cols-2 gap-2">
            {peoples?.map((user: any, i) => (
              <Link
                href={`/user/${user?._id}`}
                key={i}
                className="w-full flex items-center gap-2 border rounded-md p-2 shadow-md bg-purple-400"
              >
                <Image
